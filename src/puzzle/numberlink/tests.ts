...
export function testGenerationTime(
  width: number,
  height: number,
  pairs: number,
  testsToRun: number = 100
) {
  ...
  while (testsNumber < testsToRun) {
    ...
    if (!generationResult.puzzle) {
      ...
      if (failures === maxFailures) {
        console.log(
          `TOO MANY FAILURES when generating ${width}x${height} puzzles with ${pairs} pairs.`
        );
        return;
      }
    } else {
  ...
  const avgTime =
    successTimes.reduce((accum, time) => accum + time, 0) / successTimes.length;

  const minAttempts = minBy(successAttempts, (attempts) => attempts);
  const maxAttempts = maxBy(successAttempts, (attempts) => attempts);
  const avgAttempts =
    successAttempts.reduce((accum, attempts) => accum + attempts, 0) /
    successAttempts.length;

  console.log(
    `Ran the generation algorithm on ${testsToRun} seeds, with ${failures} failures.\n
